...
export function EntryCard({ entry, locale, vaultId }: EntryCardProps) {
  ...
  return (
    <Link href={`/${locale}/vault/${vaultId}/entries/${entry.id}`} className="block h-full">
      <div className="bg-card border border-border rounded-xl shadow-sm p-5 hover:shadow-md transition-shadow duration-200 h-full flex flex-col">
        <div className="flex items-center gap-2 mb-3">
          <CategoryIcon slug={entry.category_slug} size={18} />
          <Badge variant={entry.category_slug}>{tCat(entry.category_slug)}</Badge>
        </div>
        <h3 className="text-base font-semibold text-foreground mb-2 leading-snug line-clamp-2 flex-1">
          {entry.title}
        </h3>
        {entry.body && (
